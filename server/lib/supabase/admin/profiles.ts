...
export async function updateUserProfileByAdmin(userId: string, updates: Partial<Pick<Profile, 'first_name' | 'last_name' | 'stripe_customer_id' | 'stripe_subscription_key'>>) {
    const supabase = createSupabaseAdminClient();
    const { data, error } = await supabase
        .from('profiles')
        .update(updates)
